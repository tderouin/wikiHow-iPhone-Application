...
- (void)_selectTabForButtonIndex:(NSInteger)buttonIndex {
	if (buttonIndex == 1) {
        if (UIAppDelegate.tabBarController.selectedIndex == 0) {
            [(UINavigationController *)[UIAppDelegate.tabBarController selectedViewController] popToRootViewControllerAnimated:YES];
        } else {
            UIAppDelegate.tabBarController.selectedIndex = 0;
        }
	} else if (buttonIndex == 2) {
		if (UIAppDelegate.tabBarController.selectedIndex == 2) {
            [(UINavigationController *)[UIAppDelegate.tabBarController selectedViewController] popToRootViewControllerAnimated:YES];
        } else {
